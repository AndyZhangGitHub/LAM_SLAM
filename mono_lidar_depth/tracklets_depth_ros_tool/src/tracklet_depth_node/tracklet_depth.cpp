...
/*******************************************************
 * 
 *     1. TrackletDepth类的构造函数
 * 
*******************************************************/
TrackletDepth::TrackletDepth(ros::NodeHandle node_handle, ros::NodeHandle private_node_handle) : _image_transport{node_handle} 
{
...
void TrackletDepth::InitSubscriber(ros::NodeHandle& nh, bool use_semantics) {
    if (!use_semantics) {
        ...
        _subscriber_cloud   =     std::make_unique<SubscriberCloud>(nh, _params.subscriber_msg_name_cloud, _params.msg_queue_size);

        _subscriber_matches =     std::make_unique<SubscriberTracklets>(nh, _params.subscriber_msg_name_tracklets, _params.msg_queue_size);

        _subscriber_camera_info = std::make_unique<SubscriberCameraInfo>(nh, _params.subscriber_msg_name_camera_info, _params.msg_queue_size);
        ...
        //接收器的话题在配置文件中（config_tracklets_depth_ros_tool.yaml），其默认值为 /sensor/velodyne/cloud_euclidean
        _subscriber_cloud       =  std::make_unique<SubscriberCloud>(nh, _params.subscriber_msg_name_cloud, _params.msg_queue_size);
        //默认话题为 /matches/grayscale/left
        _subscriber_matches     =  std::make_unique<SubscriberTracklets>(nh, _params.subscriber_msg_name_tracklets, _params.msg_queue_size);
        // 默认话题为 /sensor/camera/grayscale/left/camera_info
        _subscriber_camera_info =  std::make_unique<SubscriberCameraInfo>(nh, _params.subscriber_msg_name_camera_info, _params.msg_queue_size);
        // 默认话题为 /sensor/camera/color_labels/left/image_rect
        _subscriber_semantics   =  std::make_unique<SubscriberSemantics>(nh, _params.subscriber_msg_name_semantics, _params.msg_queue_size);
...
void TrackletDepth::InitPublisher(ros::NodeHandle& nh) {
    ...
    if(_params.publisher_msg_name_cloud_camera_cs!=""){
        ...
        // 默认话题为： /monolidar_debug/cloud_camera_cs
        _publisher_cloud_camera_cs = nh.advertise<Cloud>(_params.publisher_msg_name_cloud_camera_cs, _params.msg_queue_size);
    }

    //std::cout << "Publisher name pointcloud interpolated: " << _params.publisher_msg_name_cloud_interpolated << std::endl;
    if(_params.publisher_msg_name_cloud_interpolated!=""){
        
        // 默认话题为： /monolidar_debug/cloud_interpolated
        _publisher_cloud_interpolated = nh.advertise<Cloud>(_params.publisher_msg_name_cloud_interpolated, _params.msg_queue_size);
    }
    //std::cout << "Publisher name image projection cloud: " << _params.publisher_msg_name_image_projection_cloud<< std::endl;
    if(_params.publisher_msg_name_image_projection_cloud!=""){

        // 默认话题为： /monolidar_debug/image_cloud_projection
        _publisher_image_projection_cloud = _image_transport.advertise(_params.publisher_msg_name_image_projection_cloud, _params.msg_queue_size);
    }
    ...
    if(_params.publisher_msg_name_depthcalc_stats!=""){
         // 默认话题为： /monolidar_debug/depthcalc_stats
        _publisher_depthcalc_stats = nh.advertise<std_msgs::String>(_params.publisher_msg_name_depthcalc_stats, _params.msg_queue_size);
    }
    
    //std::cout << "Publisher name tracklets depth with depth: " << _params.publisher_msg_name_tracklets_depth << std::endl;
    // 默认话题为： /tracklets_depth/tracklets/left
